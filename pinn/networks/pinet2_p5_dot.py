...
class AddLayer(tf.keras.layers.Layer):
    ...
    def call(self, tensor):
        ...
        # serial code:
        # i3 = i3[0,:,0]
        # vvt = tf.einsum('i,j->ij', i3, i3)
        # S = vvt - tf.eye(3) * tf.linalg.trace(vvt) / 3

        vvt = tf.einsum('ijl, ikl->ijkl', i3, i3)
        vvt_trace = tf.einsum('ijjl->il', vvt)  # [i,3,3,j] -> [i,j]
        vvt_shape_id = tf.transpose(tf.eye(3, batch_shape=tf.shape(vvt_trace)), perm=[0,2,3,1])
...
class GCBlock(tf.keras.layers.Layer):
    ...
    def call(self, tensors):
        ...
        p1t1 = self.dot_layer(tf.reshape(p5, (-1, 9, p5.shape[-1]))) + self.dot_layer(p3) + p1
