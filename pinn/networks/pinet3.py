...
class GCBlock(tf.keras.layers.Layer):
    def __init__(self, weighted: bool, pp_nodes, pi_nodes, ii_nodes, **kwargs):
        ...
        self.pi5_layer = PILayer(pi_nodes, **kwargs)
        ...
        self.scale_diff_layer = ScaleLayer()
    ...
    def call(self, tensors):
        ...
        scaled_diff = self.scale_diff_layer([diff[:, :, None], i1_1])
        i3 = i3 + scaled_diff
        p3 = self.ip3_layer([ind_2, p3, i3])

        p5 = self.pp5_layer(p5)
        i5 = self.pi5_layer([ind_2, p5, basis])
        i5 = self.ii5_layer(i5)
        i5 = i5 + scaled_diff
        p5 = self.ip5_layer([ind_2, p5, i5])

        p1t1 = self.dot_layer(tf.reshape(p5, ([p5.shape[0], -1, p5.shape[-1]]))) + self.dot_layer(p3) + p1
        p3t1 = self.scale3_layer([p3, p1t1])
        p5t1 = self.scale5_layer([p5, p1t1])
    ...
    def call(self, tensors):
        ...
        for i in range(self.depth):
            p1, p3 = self.gc_blocks[i](
                [tensors["ind_2"], tensors["p1"], tensors["p3"], tensors["p5"], tensors["diff"], basis]
            )
