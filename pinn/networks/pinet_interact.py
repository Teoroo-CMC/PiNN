# monkey-patch pinn to allow interaction-wise predictions
...
from pinn.networks.pinet import PiNet, PILayer, OutLayer
...
class PiNetInter(PiNet):
    def __init__(self, ppred=1, ipred=0, **kwargs):
        super(PiNetInter, self).__init__(**kwargs)
        out_nodes, depth = kwargs['out_nodes'], kwargs['depth']
        self.pout = [OutLayer(out_nodes, ppred) for i in range(depth)]
        self.iout = [PILayer(out_nodes+[ipred]) for i in range(depth)]
    ...
    def call(self, tensors):
        ...
        for i in range(self.depth):
            ...
            pout = self.pout[i]([tensors['ind_1'], prop, pout])
            iout += self.iout[i]([tensors['ind_2'], prop, basis])
        return pout, iout
...
pinn.networks.get = get_network
pinn.get_network = get_network
