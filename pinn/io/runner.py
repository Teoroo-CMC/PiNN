    ...
    with open(fname) as f:
        ...
        while line:
            ...
            if splitline[0] == "atom":
                ...
                f_data.append([float(splitline[7])/bohr2ang/ev2hartree,
                               float(splitline[8])/bohr2ang/ev2hartree,
                               float(splitline[9])/bohr2ang/ev2hartree])
                temp_f_weights = [1.0, 1.0, 1.0]
                if len(splitline) >= 13:
                    temp_f_weights = [float(splitline[10]),
                                      float(splitline[11]),
                                      float(splitline[12])]
                # change the atomic weight by elements, and use_f_weight should be set as true zzy 20240617
                # if (splitline[4] == "Na"):
                #     print("Note: the weight of Na had been changed!")
                #     temp_f_weights = [300.0,300.0,300.0]
                f_weights.append(temp_f_weights)
            elif splitline[0] == "energy":
                e_data = float(splitline[1])/ev2hartree
...
def write_runner(fname, dataset):
    ...
    for idx, data in enumerate(dataset):
        ...
        for e, c, f in zip(data['elems'], data['coord']/bohr2ang, data['f_data']*bohr2ang*ev2hartree):
            lines.append(f'atom     {c[0]:14.6e} {c[1]:14.6e} {c[2]:14.6e}  '
                         f'{chemical_symbols[e]}  '
                         f'0.0    0.0   {f[0]:14.6e} {f[1]:14.6e} {f[2]:14.6e} \n')
        lines.append(f'energy  {data["e_data"]*ev2hartree:14.6e}\n')
