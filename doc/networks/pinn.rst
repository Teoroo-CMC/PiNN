...
The primary motivation of PiNN is to design the structure
such that the interactions are more compliant to chemical intuition. Therefore, the interactions can be percieved directly in PiNN comparing to other graph convolution ANNs.
...
The interaction are meant to be a continuous function with cutoff,
...
The weights of the interaction are determined from the pairwise interaction (pi)
layer which takes as input the properties from interacting atoms.
The interactions from different neighbours are refined with a fully connected ii
